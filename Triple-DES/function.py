...
def desx(m, key1, sub_key2, key3):
    ...
    m(array): plain/cipher text 64-bit
    key1/3(array): key
    sub_key2(array): sub-key-list

    ---output---
    cipher/plain text(array)
    """
    m = np.bitwise_xor(m, key1)
    # 1. n = 1
    n = 1
    # 2. IP(m)
    IPm = IP(m)
